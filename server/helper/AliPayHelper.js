...
const ALI_PAY_SETTINGS = {
  APP_ID: config.alipay.APP_ID,
  APP_GATEWAY_URL: config.alipay.APP_GATEWAY_URL, // 用于接收支付宝异步通知
  AUTH_REDIRECT_URL: config.alipay.AUTH_REDIRECT_URL, // 第三方授权或用户信息授权后回调地址。授权链接中配置的redirect_uri的值必须与此值保持一致。
  APP_PRIVATE_KEY_PATH: config.alipay.APP_PRIVATE_KEY_PATH, // 应用私钥
  APP_PUBLIC_KEY_PATH: config.alipay.APP_PUBLIC_KEY_PATH, // 应用公钥
  ALI_PUBLIC_KEY_PATH: config.alipay.ALI_PUBLIC_KEY_PATH, // 阿里公钥
  AES_PATH: config.alipay.AES_PATH, // aes加密（暂未使用）
  RETURN_URL: config.alipay.RETURN_URL, // 回调链接
  NOTIFY_URL: config.alipay.NOTIFY_URL // 支付结果异步链接
}
class AliPayHelper {
  /**
     * 构造方法
  ...
  _buildSign(paramsMap) {
    ...
    const paramsList = [...paramsMap].filter(([k1, v1]) => k1 !== 'sign' && v1)
    // 2.按照字符的键值ASCII码递增排序
    paramsList.sort()
    // 3.组合成“参数=参数值”的格式，并且把这些参数用&字符连接起来
    const paramsString = paramsList.map(([k, v]) => `${k}=${v}`).join('&')

    const privateKey = fs.readFileSync(this.accountSettings.APP_PRIVATE_KEY_PATH, 'utf8')
    const signType = paramsMap.get('sign_type')
  ...
  verifySign(params) {
    try {
      const sign = params['sign']// 签名
      const signType = params['sign_type']// 签名类型
      const paramsMap = new Map()
      for (const key in params) {
        paramsMap.set(key, params[key])
      }
      const paramsList = [...paramsMap].filter(([k1, v1]) => k1 !== 'sign' && k1 !== 'sign_type' && v1)
      // 2.按照字符的键值ASCII码递增排序
      paramsList.sort()
      // 3.组合成“参数=参数值”的格式，并且把这些参数用&字符连接起来
      const paramsString = paramsList.map(([k, v]) => `${k}=${decodeURIComponent(v)}`).join('&')
      const publicKey = fs.readFileSync(this.accountSettings.ALI_PUBLIC_KEY_PATH, 'utf8')
   ...
   *    @returns {string}   链接地址
   */
  async buildPayUrl(outTradeNo, totalAmount, subject, body) {
    const alipaySdk = new AlipaySdk({
      appId: this.accountSettings.APP_ID,
      privateKey: fs.readFileSync(this.accountSettings.APP_PRIVATE_KEY_PATH, 'utf8'),
      gateway: this.accountSettings.APP_GATEWAY_URL
    })
    ...
    formData.addField('return_url', this.accountSettings.RETURN_URL)
    formData.addField('notify_url', this.accountSettings.NOTIFY_URL)
  ...
  checkNotifySign(body) {
    const alipaySdk = new AlipaySdk({
      appId: this.accountSettings.APP_ID,
      privateKey: fs.readFileSync(this.accountSettings.APP_PRIVATE_KEY_PATH, 'utf8'),
      alipayPublicKey: fs.readFileSync(this.accountSettings.ALI_PUBLIC_KEY_PATH, 'utf8'),
      gateway: this.accountSettings.APP_GATEWAY_URL
    })
    const verify = alipaySdk.checkNotifySign(body)
    return verify
  }
}
