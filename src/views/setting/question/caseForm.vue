<template>
  <div class="app-container">
    ...
    <el-form ref="caseForm" :model="caseForm" :rules="rules" label-width="150px" label-position="left" style="width: 400px; margin-left:50px;">
      ...
      <el-form-item label="一问几答：" prop="yiwenjida">
        <el-input v-model="caseForm.yiwenjida" />
      </el-form-item>
...
export default {
  data() {
    return {
      ...
      rules: {
        ...
        yiwenjida: [
          { required: true, message: '请输入一问几答', trigger: 'blur' }
        ],
        agent_price: [
  ...
  methods: {
    ...
    getSelects() {
      getSelects({ dept: 'question' }).then(res => {
      ...
      this.$refs['caseForm'].validate((valid) => {
        if (valid) {
          const params = {
            ...
            yiwenjida: this.caseForm.yiwenjida,
          ...
          if (this.$route.params.dialogStatus === 'update') {
            ...
            updateCase(params).then(res => {
              if (res.status === 200) {
                ...
                this.$router.push({ name: 'question-cases' })
              }
            })
          } else {
