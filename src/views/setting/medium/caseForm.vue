<template>
  <div class="app-container">
    ...
    <el-form ref="caseForm" :model="caseForm" :rules="rules" label-width="150px" label-position="left" style="width: 400px; margin-left:50px;">
      ...
      <el-form-item label="参考粉丝数：" prop="fens_num">
        <el-input v-model.number="caseForm.fens_num" />
      </el-form-item>
...
export default {
  data() {
    return {
      ...
      rules: {
        ...
        fens_num: [
          { required: true, message: '请输入参考粉丝数', trigger: 'blur' },
          { type: 'number', message: '请输入参考粉丝数', trigger: 'blur' }
        ],
        start_num: [
          { required: true, message: '请输入起始下单量', trigger: 'blur' },
          { type: 'number', message: '请输入起始下单量', trigger: 'blur' }
        ]
      }
    }
  },
  ...
  methods: {
    ...
    getSelects() {
      getSelects({ dept: 'medium' }).then(res => {
      ...
      this.$refs['caseForm'].validate((valid) => {
        if (valid) {
          const params = {
            ...
            fens_num: this.caseForm.fens_num,
          ...
          if (this.$route.params.dialogStatus === 'update') {
            ...
            updateCase(params).then(res => {
              if (res.status === 200) {
                ...
                this.$router.push({ name: 'medium-cases' })
              }
            })
          } else {
